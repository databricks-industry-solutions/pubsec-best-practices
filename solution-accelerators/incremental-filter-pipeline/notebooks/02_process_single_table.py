...
#   - Dropped columns: ⚠️ NULLs in target (source needs column mapping)
...
if raw_input:
    ...
    print("⚠️  No For Each input detected — using default test config\n")
    config = {
...
try:
    ...
    if "TABLE_OR_VIEW_NOT_FOUND" in err or "DELTA_TABLE_NOT_FOUND" in err or "is not a Delta table" in err:
        print(f"   ℹ️  Target table {tgt_fqn} does not exist yet — will be created on first write.")
    else:
        raise
